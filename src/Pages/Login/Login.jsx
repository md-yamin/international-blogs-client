    ...
    return (
        <div className="container mx-auto px-6 py-16 lg:py-24">
            <div className="mx-auto max-w-sm">
                <p className={`text-center ${labelClass}`}>Welcome Back</p>
                <h1 className="mt-3 text-center font-display italic text-4xl text-ink">Sign In</h1>
                ...
                <form onSubmit={handleSubmit(onSubmit)} noValidate="" className="mt-10 space-y-6">
                    ...
                    <div className="relative">
                        ...
                        <input type="password" name="password" id="password" placeholder="••••••••" className={`mt-2 ${fieldClass}`} {...register("password", {
                            required: true,
                            pattern: {
                                value: /^(?=.*[A-Z])(?=.*[!@#$%^&*])(?=.*\d).{6,}$/,
                    ...
                    <input type="submit" className="w-full border border-ink py-3 font-meta text-xs uppercase tracking-wide2 text-ink hover:bg-ink hover:text-paper transition-colors cursor-pointer" value="Sign in" />
                </form>
                ...
                <div className="space-y-3">
                    <button onClick={() => handleSocialLogin(signInWithGoogle)} aria-label="Login with Google" type="button" className="flex w-full items-center justify-center gap-3 border border-rule py-3 text-sm text-ink hover:border-ink transition-colors">
                    ...
                    <button onClick={() => handleSocialLogin(signInWithGithub)} aria-label="Login with GitHub" role="button" className="flex w-full items-center justify-center gap-3 border border-rule py-3 text-sm text-ink hover:border-ink transition-colors">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" className="w-4 h-4 fill-current">
                ...
                <p className="mt-10 text-center text-sm text-ink-soft">
                    ...
                    <Link to="/register" className="text-ink underline decoration-rule underline-offset-4 hover:decoration-ink transition-colors">Register here</Link>
                </p>
            </div>
        </div>
    );
